...
def selective_scan_torch(
    ...
    **kwargs
    ...
    """
    u:输入信号，形状为 (B, K * C, L)，其中：
    B 是批量大小（Batch Size）。
    K 是分组数量（Group Number）。
    C 是每组的通道数（Channel Number）。
    L 是序列长度（Sequence Length）

    delta 动态时间步长，形状为 (B, K * C, L)。它控制状态更新的速度。
    A 状态转移矩阵，形状为 (K * C, N)，其中 N 是状态空间的维度（State Dimension）
    B 输入矩阵，形状为 (B, K, N, L)，定义了输入信号如何影响状态变量的变化。
    C 输出矩阵，形状为 (B, K, N, L)，定义了状态变量如何影响输出信号
    D 直接跳过连接（Skip Connection），形状为 (K * C)。如果提供，会将输入信号直接加到输出上
    """
...
class SelectiveScanCuda(torch.autograd.Function):
    @staticmethod
    @torch.cuda.amp.custom_fwd #用于混合精度训练，后续可能弃用这个方法，可删除
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, oflex=True, backend=None):
        ctx.delta_softplus = delta_softplus
        #selective_scan_cuda_core不再使用，改用oflex
        #https://github.com/MzeroMiko/VMamba/issues/216
        backend="oflex"

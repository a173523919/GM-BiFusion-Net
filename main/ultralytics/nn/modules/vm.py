...
class SS2Dv0(nn.Module):
    ...
    def forwardv0(self, x: torch.Tensor, seq=False, force_fp32=True, **kwargs):
        ...
        y = y * z
        out = self.dropout(self.out_proj(y.to(self.out_proj.weight.dtype)))
        return out

#################行列双向扫描###################
class SS2Dv0_double(nn.Module):
    def __init__(
        self,
        # basic dims ===========
        #输入特征的维度。
    ...
    def forwardv0(self, x: torch.Tensor, seq=False, force_fp32=True, **kwargs):
        ...
        #提取out_y2，3两组数据，进行翻转，分别将按行、按列倒序扫描的两组张量恢复形状
        #inv_y = torch.flip(out_y[:, 2:4], dims=[-1]).view(B, 2, -1, L)

        #提取out_y第1组数据，进行wh置换，将按列扫描的投影张量恢复形状
        wh_y = torch.transpose(out_y[:, 1].view(B, -1, W, H), dim0=2, dim1=3).contiguous().view(B, -1, L)

        #提取inv_y第1组数据，进行wh置换，将按列扫描的投影张量恢复形状
        #invwh_y = torch.transpose(inv_y[:, 1].view(B, -1, W, H), dim0=2, dim1=3).contiguous().view(B, -1, L)
        
        y = out_y[:, 0]+ wh_y# + inv_y[:, 0]  + invwh_y
...
class SS2Dv0_ZF(nn.Module):
    ...
    def forwardv0(self, x: torch.Tensor, seq=False, force_fp32=True, **kwargs):
        ...
        x_hwwh = torch.stack([x.view(B, -1, L), torch.flip(x.view(B, -1, L), dims=[-1])], dim=1).view(B, 2, -1, L)  
        #torch.flip(x_hwwh, dims=[-1])对x_hwwh的最一个维度进行反转，生成的张量 xs 的形状为 (B, 4, D, L)
        #xs = torch.cat([x_hwwh, torch.flip(x_hwwh, dims=[-1])], dim=1) # (b, k, d, l)
        xs=x_hwwh
...
class SS2Dv0_ZJ(nn.Module):
    ...
    def forwardv0(self, x: torch.Tensor, seq=False, force_fp32=True, **kwargs):
        ...
        wh_y = torch.transpose(out_y[:, 1].view(B, -1, W, H), dim0=2, dim1=3).contiguous().view(B, 1, -1, L)
        y = out_y[:, 0] + wh_y[:, 0]
